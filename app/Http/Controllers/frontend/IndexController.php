...
class IndexController extends Controller
{
    ...
    public function productDetails($id){
        ...
        $color_ar = $product->product_color_arn;
        $product_color_ar = explode(',', $color_ar);

        $size_en = $product->product_size_en;
        $product_size_en = explode(',', $size_en);

        $size_ar = $product->product_size_hin;
        ...
        public function TagWiseProduct($tag){
            $products = Product::where('status',1)->where('products_tags_en',$tag)->where('products_tags_ar',$tag)->orderBy('id','DESC')->paginate(3);
            $categories = Category::orderBy('category_name_en','ASC')->get();
